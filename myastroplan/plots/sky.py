...
def plot_sky(target, observer, time, ax=None, style_kwargs=None,
             ...
             warn_below_horizon=False, style_sheet=None, hours_value=False):
    """
    ...
    # hours to the map added on 2018-09-26
    if hours_value is not False:
        hoursvalue = hours_value[altitude <= 91.0]
        if len(hoursvalue) > 1:
            hourname = ['%.1f' % i for i in hoursvalue]
            for i, txt in enumerate(hourname):
                ax.annotate(txt, (az_plot[i].value, alt_plot[i].value), fontsize=8)
        elif len(hoursvalue) == 1:
            hourname = ['%.1f' % i for i in hoursvalue]
            for i, txt in enumerate(hourname):
                ax.annotate(txt, (az_plot[i], alt_plot[i]), fontsize=8)

    # Plot target coordinates. # adapted by 2018-09-26
